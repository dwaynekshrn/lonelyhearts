...
const FIELDS2 = [
{type: "radio", label: "Any", name: "any", component:{Gender}},
{type: "radio", label: "Male", name: "male", component:{Gender}},
{type: "radio", label: "Female", name: "female", component:{Gender}}
]
...
class Filter extends Component{
    ...
    renderAge(){
          return _.map(FIELDS, ({type, name, values, label}) => {
           return <Field key={label} type="text" name={name} label={label} values={name} component={Age} />
          }); 
    }

    renderGender(){
        return _.map(FIELDS2, ({type, label, name}) => {
         return <Field key={label} type="radio" name={name} label={label} values={name} component={Gender} />
        }); 
  }
